...
class Battle {
 ...
 Battle(): turn_(0), description_(""), userTurns_(false), isHeroic_(false), multiattack_count_(0), scripted_(false) {}
  ~Battle() {}
  ...
  bool isHeroic() const {return isHeroic_; }
 ...
 private:
  ...
  bool isHeroic_;            // Is the boss in Heroic mode or not
  coll_type pcs_;            // Player characters
  coll_type npcs_;           // Non-player characters
  unsigned int multiattack_count_;
