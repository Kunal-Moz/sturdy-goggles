...
// Power attack : raises attack points by 20 every 3rd turn
int Entity::powerAttack( Entity * other ) {
  if ( other != 0 ) {
    setTarget(other);
  }
  ...
  if ( getTarget() != 0 ) {
    ...
    if ((turn_+1) % 3 != 0){
      ...
      ap = attackPower_ + 20;
      auto attacked = getTarget()->reduceHitPoints( ap );
      std::cout << name() << " power attacked " << getTarget()->name() << " with attack power " << ap << " for damage " << attacked << std::endl;
      if ( myAttacks_.find(turn_) == myAttacks_.end() )
        myAttacks_[turn_] = action_vector();
      myAttacks_[turn_].push_back( attacked );
      return attacked;
...
int Entity::magicAttack(Entity * other){
  if ( other != 0 ) {
    setTarget(other);
  }
  ...
  if ( getTarget() != 0 ) {
    ...
    int ap = attackPower_;
      if ((turn_+1) % 4 != 0){
        auto attacked = getTarget()->reduceHitPoints( ap );
        std::cout << name() << " casts a spell " << getTarget()->name() << " with attack power " << ap << " for damage " << attacked << std::endl;
        if ( myAttacks_.find(turn_) == myAttacks_.end() )
          myAttacks_[turn_] = action_vector();
        myAttacks_[turn_].push_back( attacked );
        return attacked;
        ...
        ap = attackPower_ + 14;
        auto attacked = getTarget()->reduceHitPoints( ap );
        std::cout << name() << " called the spirits of Meraxes " << getTarget()->name() << " with attack power " << ap << " for damage " << attacked << std::endl;
        if ( myAttacks_.find(turn_) == myAttacks_.end() )
          myAttacks_[turn_] = action_vector();
        myAttacks_[turn_].push_back( attacked );
        return attacked;
